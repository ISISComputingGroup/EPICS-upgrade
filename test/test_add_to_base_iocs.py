...
class TestAddToBaseIOCs(unittest.TestCase):
    ...
    def test_GIVEN_invalid_xml_WHEN_adding_ioc_THEN_error(self):
        ...
        assert_that(
            self.logger.log_err,
            has_item("IOC file appears not be valid XML, error '{}'".format(error_str)),
        )
    ...
    def test_GIVEN_xml_already_containing_ioc_to_add_WHEN_checking_prerequisites_THEN_error(self):
        ...
        self._assert_prerequiste_fails(
            adder, xml, ALREADY_CONTAINS.format(FILE_TO_CHECK_STR, ioc_to_add)
        )

    def test_GIVEN_xml_already_containing_two_ioc_to_add_WHEN_checking_prerequisites_THEN_error(
        self,
    ):
        ioc_to_add = "TO_ADD"
    ...
    def test_GIVEN_xml_containing_two_ioc_to_add_after_WHEN_checking_prerequisites_THEN_error(self):
        ...
        adder = AddToBaseIOCs("", after_ioc, "")
        self._assert_prerequiste_fails(
            adder, xml, ADD_AFTER_MISSING.format(FILE_TO_CHECK_STR, 2, after_ioc)
        )

    def test_GIVEN_xml_containing_ioc_to_add_after_and_no_ioc_to_add_WHEN_checking_prerequisites_THEN_passes(
        self,
    ):
        ioc_to_add = "TO_ADD"
        after_ioc = "AFTER_THIS"
