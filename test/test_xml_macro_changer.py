...
class TestMacroChangesWithMultipleInputs(unittest.TestCase):
    ...
    def test_that_GIVEN_xml_with_single_macro_WHEN_calling_change_macros_THEN_the_single_macro_is_updated(self):
        ...
        ioc_name = "GALIL"
        macro_to_change = [
            (Macro("GALILADDRXX"), Macro("GALILADDR"))
        ]
...
class TestMacroManipulations(unittest.TestCase):
    ...
    def test_than_GIVEN_xml_with_old_ioc_macros_WHEN_manipulate_macros_THEN_macros_manipulated(self):
        # Given:
        xml = IOC_FILE_XML.format(iocs=create_galil_ioc(1, {"GALILADDR": "1", "MTRCTRL": "1"}))
        ioc_name = "GALIL"
        add_at_symbol = lambda value: f"@{value}"
        macros_to_change = [
            (Macro("GALILADDR"), add_at_symbol),
            (Macro("MTRCTRL"), add_at_symbol)
        ]
        self.file_access.write_file_contents = xml
        ...
        # When:
        self.macro_changer.manipulate_macro_values(ioc_name, macros_to_change)
    ...
    def test_than_GIVEN_xml_with_one_old_ioc_macros_WHEN_manipulate_macros_THEN_only_one_macro_manipulated(self):
        # Given:
        xml = IOC_FILE_XML.format(iocs=create_galil_ioc(1, {"GALILADDR": "1", "MTRCTRL": "1"}))
        ioc_name = "GALIL"
        add_at_symbol = lambda value: f"@{value}"
    ...
    def test_than_GIVEN_xml_with_different_ioc_WHEN_manipulate_macros_THEN_no_macros_manipulate(self):
        # Given:
        xml = IOC_FILE_XML.format(iocs=create_galil_ioc(1, {"GALILADDR": "1", "MTRCTRL": "1"}))
        ioc_name = "ILM200"
        add_at_symbol = lambda value: f"@{value}"
