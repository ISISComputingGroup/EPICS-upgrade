...
class TestUpgradeStepFrom3p2p1p1(unittest.TestCase):

    def setUp(self):
        self.file_access = FileAccessStub()
        self.upgrade_step = UpgradeStepFrom3p2p1p1()
    ...
    def test_GIVEN_ioc_xml_file_WHEN_standard_THEN_file_changed(self):
        ...
        assert_that(self.file_access.write_file_contents.replace('\n', ''),
                    equal_to_ignoring_whitespace(mother.CLEAN_COMPONENT_BASE_IOC_FILE_v3p2p1p2.replace('\n', '')))

    def test_GIVEN_ioc_xml_file_has_araccess_ioc_in_already_WHEN_upgrade_THEN_error(self):

        self.file_access.open_file = Mock(return_value=mother.CLEAN_COMPONENT_BASE_IOC_FILE_v3p2p1p2)
    ...
    def test_GIVEN_ioc_xml_file_with_too_many_ARACCESS_in_WHEN_upgrade_THEN_error(self):

        self.file_access.open_file = Mock(return_value=mother.ERROR_COMPONENT_BASE_IOC_FILE_TWO_ARACCESS)
